...
create_fof() {
    local input_dir="$WORKDIR/visor_pacbio/input_vcf"
    local fof_file="$WORKDIR/visor_pacbio/svmerge_4callers_files.txt"
...
run_svmerge() {
    local fof_file=$1
    local output_dir="$WORKDIR/visor_pacbio/union/svmerge_4callers"
    
    echo "Running SVmerge for visor_pacbio"
...
# Main process
echo "Starting SVmerge analysis for visor_pacbio with 4 callers"
fof_file=$(create_fof)
run_svmerge "$fof_file"
rm "$fof_file"
echo "SVmerge analysis completed for visor_pacbio"
